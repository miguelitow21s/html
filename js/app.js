...
const app = {
    ...
    renderEmployeeDashboard() {
        ...
        this.renderEmployeeOtherTodayShifts();
        this.updateUserUI();
    },

    renderEmployeeOtherTodayShifts() {
        const section = document.getElementById('employee-other-today-shifts-section');
        const list = document.getElementById('employee-other-today-shifts-list');
        ...
        // Otros turnos del día = todos los today_shifts excepto el que ya se muestra en la card principal
        // y excepto los cerrados. Si solo hay 1 (el actual), se oculta la sección.
        const others = todayShifts.filter((shift) => {
            const shiftId = String(shift?.shift_id || shift?.scheduled_shift_id || shift?.id || '').trim();
            if (!shiftId) return false;
            if (shiftId === currentId) return false;
        ...
        if (!list.dataset.delegationReady) {
            list.dataset.delegationReady = '1';
            list.addEventListener('click', (event) => {
                const button = event.target.closest('[data-other-shift-id]');
                if (!button) return;
                const targetId = String(button.dataset.otherShiftId || '').trim();
                if (!targetId) return;
                const all = asArray(this.data.employee.todayShifts);
                const picked = all.find((shift) => {
                    const sid = String(shift?.shift_id || shift?.scheduled_shift_id || shift?.id || '').trim();
                    return sid === targetId;
                });
                if (!picked) return;
                // Reemplazar el pending por el turno elegido. Si es un activo, va a currentShift.
                const state = String(picked?.state || picked?.status || '').toLowerCase();
                if (state === 'activo' || state === 'active' || state === 'in_progress') {
                    this.data.currentShift = this.enrichEmployeeShiftRecord?.(picked, this.data.employee.dashboard) || picked;
                    this.data.currentScheduledShift = null;
                } else {
                    this.data.currentShift = null;
                    this.data.currentScheduledShift =
                        this.enrichEmployeeShiftRecord?.(picked, this.data.employee.dashboard) || picked;
                }
                this.renderEmployeeDashboard();
        ...
        list.innerHTML = others
            .map((shift) => {
                const restaurantName =
                    ...
                    'Sitio sin nombre';
                const timeRange = formatShiftLocalRange(shift);
                const state = String(shift?.state || shift?.status || '').toLowerCase();
                const activeBadge =
                    state === 'activo' || state === 'active' || state === 'in_progress'
                        ? '<span class="badge badge-success" style="margin-left:8px;">Activo</span>'
                        : '';
                const shiftId = String(shift?.shift_id || shift?.scheduled_shift_id || shift?.id || '').trim();
                return `<button type="button" class="other-shift-item" data-other-shift-id="${escapeHtml(shiftId)}" style="display:block;width:100%;text-align:left;padding:12px 14px;margin-bottom:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:inherit;cursor:pointer;">
                    <div style="font-weight:600;margin-bottom:4px;">
                        <i class="fas fa-store" style="margin-right:6px;opacity:0.7;"></i>${escapeHtml(restaurantName)}${activeBadge}
                    </div>
                    <div class="muted-copy" style="font-size:12px;">
                        <i class="fas fa-clock" style="margin-right:4px;opacity:0.7;"></i>${escapeHtml(timeRange)}
                    </div>
                </button>`;
            })
            .join('');
