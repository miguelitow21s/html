...
- `403 PERMISSION`: role mismatch or missing legal consent.
...
Shift OTP header
- `super_admin` no longer needs `x-shift-otp-token` for `shifts_approve`, `shifts_reject`, or `incidents_create`.
- Those endpoints now work with authenticated session only: `Authorization` + `apikey`.
...
3. Continue with authenticated session.
No trusted-device validation or OTP is required for `super_admin` in approval/rejection/incident flows.
...
Headers (plus global)
- Session auth only (`Authorization` + `apikey`)
...
2. Manage users and roles with `admin_users_manage`.
3. Manage restaurants and geolocation rules with `admin_restaurants_manage`.
4. Assign supervisors with `admin_supervisors_manage`.
5. Schedule shifts and bulk schedules with `scheduled_shifts_manage`.
6. Review and approve or reject shifts with `shifts_approve` or `shifts_reject`.
7. Create incidents with `incidents_create` when needed.
8. Monitor metrics and costs with `admin_dashboard_metrics`.
9. Manage supplies with `supplies_deliver`.
10. Generate reports with `reports_generate`.
