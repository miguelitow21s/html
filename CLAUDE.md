...
- **Duplicaciones de funciones entre app.js y adminModals.js**: mismo namespace `WorkTraceApp`; la última cargada pisa. Bug histórico repetido (populateSupervisorAreaOptions, setSupervisorSelectedArea, resetSupervisorSupervisionState). Al agregar métodos verificar que no exista otra definición del mismo nombre. Consolidadas en app.js (PR #16, #22). En 2026-09 se unificaron 28 más (cronómetro app↔employee, getKnown*/getResolved* app↔supervisor↔adminModals, etc.): lo de varios roles quedó en app.js y lo de inspector+admin en supervisor.js. Orden de carga: inspector = app → supervisor → adminModals; super_admin = app → supervisor → admin → adminModals.
